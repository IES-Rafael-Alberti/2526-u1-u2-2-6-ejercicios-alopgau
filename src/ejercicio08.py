...
def validar_contrasena(contrasena: str) -> tuple[bool, int, int, int, int, int]:
    ...
    while (tiene_minuscula != 1 and tiene_digito != 1 and tiene_mayuscula != 1 and tiene_especial != 1) or i <= len(contrasena):
        for digito in contrasena:
            if digito.isdigit():
                ...
                tiene_especial = 1
            i += 1
    if tiene_longitud == 1 and tiene_mayuscula == 1 and tiene_digito == 1 and tiene_minuscula == 1 and tiene_especial == 1:
        es_valida = True
    else:
        es_valida = False
    return es_valida, tiene_longitud, tiene_mayuscula, tiene_minuscula, tiene_digito, tiene_especial
